...
# SQL 접속 설정
conn = pymysql.connect(host='localhost', user='pi', passwd='***', db='arxiv_scrape')
cur = conn.cursor()
...
def job_function():
    ...
    for i in range(len(results)):
        cur.execute(
            "INSERT INTO pages (date, title, author, category, link) VALUES (%s, %s, %s, %s, %s)",
            (str(date), results[i]['title'], results[i]['authors'], results[i]['category'], results[i]['link'])
            )
